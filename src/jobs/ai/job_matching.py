        ...
        try:
            ...
            if(combined_score>=.30):
                final_results.append({
                    ...
                    "proposal_status": Proposalai.objects.filter(job=job, user=current_user).values('status').first().get('status') if Proposalai.objects.filter(job=job).exists() else None,
                })
        except Exception as e:
